...
if __name__ == '__main__':
  ...
  data_diag.columns=["subject_id", "admittime", "dischtime", "interpretation", "diagnosis", "seq_num", "num_prod"]
  ...
  for Pt, group in n_data.groupby('subject_id'):
    ...
    #   pt_los=[]
      ...
      pt_ls=[]
      v=0
      for Time, subgroup in group.sort_values(["seq_num", "num_prod"], ascending=True).groupby('dischtime', sort=False): ### changing the sort order
          v=v+1
          ...
          pt_ant = np.array(subgroup["interpretation"].drop_duplicates()).tolist()
          print("patients antibiotic", pt_ant)
          ant_seq.extend(pt_ant)
          print("antibiotic sequence", ant_seq)
          pt_discdt.append((dt.strptime(Time, '%Y-%m-%d')))
          pt_addt.append(dt.strptime(min(np.array(subgroup['admittime'].drop_duplicates()).tolist()), '%Y-%m-%d'))                
      ...
      pts_ls.append(pt_ls)
      pts_sls.append([Pt,ant_seq[-1],full_seq,v_seg])
