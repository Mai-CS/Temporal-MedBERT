...
flags.DEFINE_integer("max_seq_length", 70, "Maximum sequence length.")
...
flags.DEFINE_string(
    "vocab_file",
    None,
    "The vocabulary file / types file that the BERT model was trained on.",
)
...
def write_EHRinstance_to_example_files(
    seqs,
    max_seq_length,
    max_predictions_per_seq,
    masked_lm_prob,
    ...
    for (seq_index, seq) in enumerate(seqs):
        ...
        # features = {key.replace('\'', '\"'):val for key, val in features.items()}
...
)  ## mainly used to sort the data below


def create_masked_EHR_predictions(
    input_seq, masked_lm_prob, max_predictions_per_seq, vocab, rng
):
    ...
    num_to_predict = min(
        max_predictions_per_seq, max(1, int(round(len(input_seq) * masked_lm_prob)))
    )
    ...
    for index in cand_indexes:
        ...
        masked_lms.append(
            MaskedLmInstance(index=index, label=input_seq[index])
        )  ### Currently keeping the original code but I need to optimize that later from here till end of function
...
def main(_):
    ...
    write_EHRinstance_to_example_files(
        train_data,
