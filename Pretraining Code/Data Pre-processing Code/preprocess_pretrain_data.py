...
def split_fn(pts_ls,pts_sls,outFile):
   ...
   all_ptencs = [pts_ls[i] for i in ind]
   all_ptencs_s =[pts_sls[i] for i in ind]
   ptencsfile = outFile +'.ptencs.'+"all"
   bertencsfile = outFile +'.bencs.'+"all"
   pickle.dump(all_ptencs, open(ptencsfile, 'a+b'), -1)
   pickle.dump(all_ptencs_s, open(bertencsfile, 'a+b'), -1)
...
if __name__ == '__main__':
  ...
  data_diag.columns=["subject_id", "admittime", "dischtime", "diagnosis", "seq_num", "num_prod"]
  ...
  for Pt, group in n_data.groupby('subject_id'):
      ...
      pt_ls=[]
      v=0
      for Time, subgroup in group.sort_values(["dischtime", "seq_num", "num_prod"], ascending=True).groupby('dischtime', sort=False): ### changing the sort order
          v=v+1
